...
- **Descrição:** Utilização de um algoritmo de criptografia customizado (`badCrypto`) baseado em codificação Base64 sem salting para armazenar senhas de usuários no banco de dados.
...
- **Arquivo:** [src/utils.js](../ecommerce-api-legacy/src/utils.js#L1-L7)
- **Descrição:** Chaves privadas, tokens e credenciais de produção (`paymentGatewayKey`, `dbPass`, `dbUser`, etc.) estão chumbadas diretamente no código fonte do arquivo `utils.js`.
...
- **Arquivo:** [src/AppManager.js](../ecommerce-api-legacy/src/AppManager.js#L1-L142)
...
- **Descrição:** A conexão SQLite é aberta em memória (`:memory:`) diretamente no construtor do `AppManager`, o que faz com que todos os dados gravados sejam perdidos ao reiniciar o servidor, além de compartilhar uma única conexão síncrona globalmente.
...
- **Arquivo:** [src/AppManager.js](../ecommerce-api-legacy/src/AppManager.js#L89-L125)
...
- **Recomendação:** Habilitar chaves estrangeiras (`PRAGMA foreign_keys = ON;`) no SQLite e configurar a remoção correspondente em cascata no banco ou de forma transacional.
...
- **Descrição:** A porta do servidor está fixada como `3000` em um arquivo de configurações sem buscar fallbacks dinâmicos.
...
- **Recomendação:** Alterar a inicialização para ler `process.env.PORT` e permitir a configuração dinâmica da porta.

### [LOW] Uso de Valores Mágicos Literais (Inline Domain Constants / Magic Strings)
- **Arquivo:** [src/AppManager.js](../ecommerce-api-legacy/src/AppManager.js#L46) (Linhas 46, 54, 108)
...
- **Impacto:** Facilita a ocorrência de erros de digitação silenciosos e dificulta a manutenção do domínio.
- **Recomendação:** Agrupar esses status e constantes em um arquivo unificado como `src/config/constants.js`.
